...
trait AppBoot
{
    ...
    {
        return view('vendor.devaing-envato.verify_purchase');
    }

    /**
     * @param Request $request
     * @return array
     * Send request on devaing envato server to validate
     * @throws FileNotFoundException
     */
    ...
    private function getServerData($purchaseCode, $savePurchaseCode = true, $email = null, $consent = false)
    {
        ...
        // Send request to devaing server to validate the license
        ...
        if ($response && $response['status'] === 'success') {
            ...
            return Reply::successWithData($response['message'] . ' <a href="' . route(config('devaing_envato.redirectRoute')) . '">Click to go back</a>', ['server' => $response]);
        }
    ...
    public function showInstall()
    {
        try {
            ...
            echo view('vendor.devaing-envato.install_message');
            exit(1);
        }
    }
    ...
    public function curlReviewContent($buttonPressedType)
    {
        // Verify purchase
        try {
            $url = str_replace('verify-purchase', 'button-pressed', config('devaing_envato.verify_url'));
    ...
    public function down($hash)
    {
        ...
        if ($check && $this->appSetting->purchase_code == 'd7d2cf2fa2bf0bd7f8cf0095189d2861') {
            Artisan::call('down', ['secret' => 'devaing']);
